...
const transWorker = new Worker(`${window.location.origin}/vendors/transcode.worker.js`);
// APPID，APISecret，APIKey在控制台-我的应用-语音听写（流式版）页面获取
const APPID = '5fa91207';
const API_SECRET = '1d8e9280562d57782a43921ce4120b5a';
const API_KEY = 'dd92f5e2a80d5de461d1f0d5a257c799';
...
function getWebSocketUrl() {
  return new Promise((resolve, reject) => {
    ...
    const postUrl = `${url}?authorization=${authorization}&date=${date}&host=${host}`;
    resolve(postUrl);
  });
}
...
class IatRecorder {
  ...
  // 修改录音听写状态
  setStatus(status) {
    if (this.onWillStatusChange && this.status !== status) {
  ...
  setResultText({ resultText, resultTextTemp } = {}) {
    if (this.onTextChange) {
      this.onTextChange(resultTextTemp || resultText || '');
    }
    if (resultText !== undefined) {
      this.resultText = resultText;
    }
    if (resultTextTemp !== undefined) {
  ...
  connectWebSocket() {
    return getWebSocketUrl().then(url => {
      let iatWS;
      if (window.WebSocket) {
        iatWS = new window.WebSocket(url);
      } else if (window.MozWebSocket) {
        iatWS = new window.MozWebSocket(url);
      } else {
  ...
  recorderStop() {
    ...
    if (!(/Safari/.test(navigator.userAgent) && !/Chrome/.test(navigator.userAgen)) && this.audioContext) {
